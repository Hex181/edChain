export const idlFactory = ({ IDL }) => {
  const CoursePayload = IDL.Record({
    'title' : IDL.Text,
  ...
  const Course = IDL.Record({
    ...
    'creator_principal' : IDL.Text,
  });
  const LessonPayload = IDL.Record({
    'title' : IDL.Text,
    'content' : IDL.Text,
  });
  const Error = IDL.Variant({
    'InputValidationFailed' : IDL.Record({ 'msg' : IDL.Text }),
  ...
  return IDL.Service({
    'add_course' : IDL.Func([CoursePayload], [IDL.Opt(Course)], []),
    'add_lesson' : IDL.Func([LessonPayload, IDL.Nat64], [Result], []),
    ...
    'update_course' : IDL.Func([IDL.Nat64, CoursePayload], [Result_2], []),
    'update_lesson' : IDL.Func([IDL.Nat64, LessonPayload], [Result_3], []),
